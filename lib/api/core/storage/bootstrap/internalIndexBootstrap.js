...
const
  debug = require('../../../../kuzzleDebug')('kuzzle:bootstrap:internalIndex'),
...
class InternalIndexBootstrap extends SafeBootstrap {
  ...
  async _bootstrapSequence () {
    ...
    await this._indexStorage.create(
      ...
      { content: { version: this.dataModelVersion } });
  }
  ...
  _throwLockWaitTimeout () {
    errorsManager.throw('services', 'storage', 'bootstrap_timeout', 'internalIndex');
  }
  ...
  async _getJWTSecret () {
    ...
    return response._source.content.seed;
  }
  ...
  async _persistJWTSecret () {
    ...
    try {
      await this._indexStorage.create('config', this._JWT_SECRET_ID, { content: { seed } });
    }
    catch (e) {
  ...
  async _loadApiKeys () {
    ...
    await ApiKey.batchExecute({ match_all: {} }, apiKeys => {
      for (const { _source } of apiKeys) {
        promises.push(this._kuzzle.repositories.token.persistForUser(
          _source.token,
          _source.userId,
          _source.ttl));
      }
