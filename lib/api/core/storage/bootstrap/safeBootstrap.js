...
const
  debug = require('../../../../kuzzleDebug')('kuzzle:bootstrap'),
...
class SafeBootstrap {
  ...
  async startOrWait () {
    ...
    if (bootstrapDone) {
      debug('Bootstrap already done. Skipping.');
      return;
    }
  ...
  async _isLocked () {
    ...
    try {
      ...
      await this._indexStorage.create(
        'config',
        this._LOCK_ID,
        { content: { timestamp: Date.now(), refresh: 'wait_for' } });
    ...
    if (lock._source.content.timestamp < thirtySecondsAgo) {
