...
const
  ...
  Postgresql = require('../../../services/postgresql'),
...
class StorageEngine {
  constructor(kuzzle) {
    ...
    this._publicClient = new ClientAdapter(
      new Postgresql(this._kuzzle, this.config, 'public'),
      this.indexCache);

    // Storage client for internal indexes only
    this._internalClient = new ClientAdapter(
      new Postgresql(this._kuzzle, this.config, 'internal'),
      this.indexCache);
  }
  ...
  _add ({ index, collection, scope='public', notify=true } = {}) {
    ...
    if (index) {
      ...
      if (collection && ! this._indexes[index].collections.includes(collection)) {
