...
const
  debug = require('../../kuzzleDebug')('kuzzle:funnel'),
...
class FunnelController {
  ...
  execute(request, callback) {
    ...
    if (!this.kuzzle.router.isConnectionAlive(request.context)) {
      ...
      callback(processError.get('connection_dropped'));
      return 0;
    }
      ...
      .then(isAllowed => {
        if (!isAllowed) {
          ...
          if (request.context.token.userId === -1) {
            error = errorsManager.get('security', 'rights', 'unauthorized');
          }
          else {
            // logged-in user with insufficient permissions => 403 (Forbidden) error
            error = errorsManager.get('security', 'rights', 'forbidden');
          }

          request.setError(error);

          return this.kuzzle
            .pipe('request:onUnauthorized', request)
